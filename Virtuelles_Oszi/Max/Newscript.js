...
function newElement(){
  ...
  var canvas = document.getElementById('Element1');       //variable canvas holt sich die Canvas Bezeichnung "Element1 aus V1.html"
  let ctx = canvas.getContext('2d');                      //notwendig, um vorgefertigte Methoden im Canvas zu verwenden
  let XposMouse,YposMouse;                               

 // Verwendete Canvas-Größe: 1350 * 650
  var objekts = []; // g einfügen zum zeigen

 //RotateButton: constructor(posX,posY,radius,stepValue,value,index,ctx,klickfunktion,minValue,maxValue)
 // EckigerButton: constructor(posX,posY,widthX,widthY,index,ctx,klickfunktion,fillstyle)
  objekts.push(new NormalRotateButton(820,240,15, 1, 0, 3,ctx,true,-100,100)); // Triggerlevel Drehknopf
  objekts.push(new NormalRotateButton(1100,240,15, 1, 0, 4,ctx,true,-100,100)); //Curser Drehknopf
  objekts.push(new NormalRotateButton(730,345,15, 1, 0, 5,ctx,true,-100,100)); //intensity Drehknopf

  let a=new AbhängigerButton(990,80,15, 100, 0,ctx,true,"X")//kleiner Drehknopf Horizonza-
  let b=new AbhängigerButton(750,600,15, 100,0,ctx,true,"Y")//kleiner Grehknopf Kanal 1
  let c=new AbhängigerButton(925,600,15, 100,0,ctx,true,"Y")//kleiner Grehknopf Kanal 2
  let d=new AbhängigerButton(1100,600,15, 100,0,ctx,true,"Y")//kleiner Grehknopf Kanal 3
  let e=new AbhängigerButton(1275,600,15, 100,0,ctx,true,"Y")//kleiner Grehknopf Kanal 4
  ...
  objekts.push(new AchsenSkalierungButton(725,70,30, 1, 0,ctx,0,a,10**-9,50,"s",50)); //großer Drehknopf Horizontal

  objekts.push(new AchsenSkalierungButton(750,465,30, 1, 0,ctx,0,b,10**-3,5,"v",5));// großer Drehknopf Kanal 1
  objekts.push(new AchsenSkalierungButton(925,465,30, 1, 0,ctx,0,c,10**-3,5,"v",5));// großer Drehknopf Kanal 2
  objekts.push(new AchsenSkalierungButton(1100,465,30, 1, 0,ctx,0,d,10**-3,5,"v",5));// großer Drehknopf Kanal 3
  objekts.push(new AchsenSkalierungButton(1275,465,30, 1, 0,ctx,0,e,10**-3,5,"v",5));// großer Drehknopf Kanal 4
